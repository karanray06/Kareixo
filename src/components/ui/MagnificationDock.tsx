...
function DockItem({
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={cn("relative inline-flex items-center justify-center rounded-sm bg-surface border-border border cursor-pointer transition-colors hover:border-structural hover:text-structural text-text-secondary", className)}
      tabIndex={0}
      role="button"
      aria-haspopup="true"
    >
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={cn("absolute -top-8 left-1/2 w-fit whitespace-pre rounded-sm border border-border bg-surface px-2 py-1 font-badge-mono text-badge-mono text-fg-default", className)}
          role="tooltip"
          style={{ x: '-50%' }}
        >
          {children}
